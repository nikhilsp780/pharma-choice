...
class Dbtransactions:
    ...
    def getAllCategories(self):
        ...
        db.execute("SELECT id, category_name FROM categories ORDER BY category_name")
        rs = db.fetchall()
        con.close()
    ...
    def placeOrder(self, user_id):
        ...
        for item in cart_items:
            query = """
            INSERT INTO orders(user_id, drug_id, quantity, total_amount, status)
            VALUES(%s, %s, %s, %s, 'Order Placed')
            """
            db.execute(query, (user_id, item['drug_id'], item['quantity'], item['item_total']))
    ...
    def getUserOrders(self, user_id):
        ...
        # Removed o.price and o.order_date
        query = """
        SELECT o.id, d.drug_name, c.category_name, o.quantity, o.total_amount, o.status
        FROM orders o
